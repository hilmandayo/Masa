...
def main():
    for data_id in Path(args["input"]).iterdir():
        video_file = list(data_id.rglob("*.mp4"))
        csv_file = list(data_id.rglob("annotations.csv"))
        if len(video_file) != 1 or len(csv_file) != 1:
        ...
        extracted_dir = video_file.parent / ".extracted"
...
def extract_data(video_file, csv_file):
